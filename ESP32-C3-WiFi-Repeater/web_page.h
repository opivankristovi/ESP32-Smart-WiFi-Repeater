...
<head>
  ...
  <title>ESP32-C3 Smart Repeater</title>
...
// Input type select: show the digital (active-low) row or the analog
// (scaling + calibration + threshold) row.
function toggleInput(n) {
  const sel = document.getElementById('in' + n + '_type');
  if (!sel) return;
  const analog = (sel.value === '1');
  const dig = document.getElementById('in' + n + '_dig');
  const ana = document.getElementById('in' + n + '_ana');
  if (dig) dig.style.display = analog ? 'none' : '';
  if (ana) ana.style.display = analog ? '' : 'none';
  if (analog) toggleInputCal(n);
}

// Show the raw min/max calibration block only when "Percent" scaling is chosen.
function toggleInputCal(n) {
  const sel = document.getElementById('in' + n + '_scale');
  const cal = document.getElementById('in' + n + '_cal');
...
async function setCal(ch, field) {
  try {
    ...
    const el = document.getElementById('in' + (ch + 1) + '_' + field);
    if (el) el.value = v;
  } catch (e) { /* ignore */ }
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  [['bme_en','bme_body'], ['ds_en','ds_body'],
   ['in1_en','in1_body'], ['in2_en','in2_body'],
   ['in3_en','in3_body'], ['in4_en','in4_body']]
    .forEach(([cb, box]) => {
  ...
  toggleI2c();
  toggleProbe();
  for (let i = 1; i <= 4; i++) toggleInput(i);
  modeChanged(1);
  modeChanged(2);
});
</script>
