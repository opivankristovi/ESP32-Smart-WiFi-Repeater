...
static String toJson() {
  ...
  for (int i = 0; i < NUM_INPUTS; i++) {
    ...
    in["name"]  = ic.name;
    in["al"]    = ic.activeLow;
    in["scale"] = (int)ic.scale;
    in["rmin"]  = ic.rawMin;
    in["rmax"]  = ic.rawMax;
    thrToJson(in["t"].to<JsonObject>(), ic.thr);
  }
...
static void fromJson(const String& json) {
  ...
  if (inArr) {
    ...
    for (JsonObjectConst in : inArr) {
      ...
      config.inputs[i].scale     = (AnalogScale)(in["scale"] | 0);
      config.inputs[i].rawMin    = in["rmin"] | 0;
      config.inputs[i].rawMax    = in["rmax"] | 4095;
      thrFromJson(in["t"], config.inputs[i].thr);
      i++;
    }
  }
