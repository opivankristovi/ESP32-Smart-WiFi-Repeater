...
static String sourceOptions(MetricSource sel) {
  struct { MetricSource v; const char* n; } items[] = {
      ...
      {SRC_INPUT1, "Input 1"},       {SRC_INPUT2, "Input 2"},
      {SRC_INPUT3, "Input 3"},       {SRC_INPUT4, "Input 4"}};
  String o;
  for (auto& it : items) {
...
static String appShell() {
  ...
  String s = "<header class=\"hdr\"><div class=\"appbar\">"
             ...
             "<h1>ESP32-C3 Smart Repeater</h1></span>"
...
static String sensorsCard() {
  ...
  s += "<fieldset><legend>I2C sensor (GPIO6/7)</legend>";
  s += checkbox("bme_en", "Enabled", config.i2c.enabled,
                "Barometric sensor on the I2C bus (SDA 6, SCL 7).");
  s += "<div id=\"bme_body\">";
  {
  ...
  s += "<fieldset><legend>Temperature probe (GPIO5)</legend>";
  s += checkbox("ds_en", "Enabled", config.probe.enabled,
                "Temperature probe on GPIO5. Both the DS18B20 and the DHT "
  ...
  // Inputs 1–4 (each configurable as digital button or analog sensor)
  ...
  for (int i = 0; i < NUM_INPUTS; i++) {
    ...
    s += checkbox(p + "_en", "Enabled", in.enabled,
                  "Digital inputs are published as a binary_sensor and can "
                  "drive a relay in Button toggle mode. Analog inputs are "
                  "sampled, scaled, and published as sensor values.");
    s += "<div id=\"" + p + "_body\">";
    s += textField(p + "_name", "Name", in.name, "text", "",
                   "Friendly name shown in live readings and Home Assistant "
                   "(e.g. \"Hallway switch\" or \"Light level\").");
    s += "<label for=\"" + p + "_type\">Type" +
         info("Digital button = a momentary switch to GND. Analog sensor = "
              "read ADC value (e.g. light sensor, potentiometer).") +
         ...
         String(in.type == INPUT_ANALOG ? " selected" : "") +
         ">Analog sensor</option></select>";
    ...
    s += "<div id=\"" + p + "_dig\">" +
         ...
         "</div>";
    // Analog-only: scaling, calibration, threshold.
    s += "<div id=\"" + p + "_ana\">";
    s += "<label>Scaling" +
         info("Raw = ADC counts 0-4095. Percent = map raw min/max to 0-100%. "
              "Voltage = measured volts.") +
         "</label><select id=\"" + p + "_scale\" name=\"" + p +
         "_scale\" onchange=\"toggleInputCal(" + n + ")\">";
    for (int k = 0; k < 3; k++) {
      s += "<option value=\"" + String(k) + "\"" +
           ((int)in.scale == k ? " selected" : "") + ">" + scaleNames[k] +
           "</option>";
    }
    s += "</select>";
    s += "<div id=\"" + p + "_cal\">";
    s += "<div class=\"row\"><div>" +
         textField(p + "_rmin", "Raw min (0%)", String(in.rawMin), "number", "",
                   "ADC count that maps to 0%. Tip: expose the sensor to its "
                   "lowest input, then press \"Set as min\".") +
         "</div><div>" +
         textField(p + "_rmax", "Raw max (100%)", String(in.rawMax), "number", "",
    ...
    s += "</div>";  // end cal block
    s += thrFields(p + "_t", in.thr,
                   String(in.name).length() ? String(in.name) : ("Input " + n),
                   analogUnit(in.scale));
    s += "</div>";  // end ana block
  ...
  for (int i = 0; i < NUM_RELAYS; i++) {
    ...
    s += "<label>Mode" +
         info("Off = always off. Manual = controlled by you / MQTT. "
              ...
              "Button toggle = flip on each press of the selected input. "
    ...
    // Button-toggle mode: select which input drives this relay.
    {
      String btn = "<label>Toggle input" +
                   info("Which input toggles this relay on each press.") +
                   "</label><select name=\"" + p + "_btn\">";
      for (int k = 0; k < NUM_INPUTS; k++) {
        btn += "<option value=\"" + String(k) + "\"" +
               (r.buttonInput == k ? " selected" : "") +
               ">Input " + String(k + 1) + "</option>";
      }
      btn += "</select>";
      s += modeSec(n, RELAY_BUTTON, r.mode, btn);
    }
...
static void handleSensors() {
  ...
  for (int i = 0; i < NUM_INPUTS; i++) {
    ...
    in.scale = (AnalogScale)server.arg(p + "_scale").toInt();
    in.rawMin = server.arg(p + "_rmin").toInt();
    in.rawMax = server.arg(p + "_rmax").toInt();
    parseThr(p + "_t", in.thr);
  }
...
static void handleRelays() {
  for (int i = 0; i < NUM_RELAYS; i++) {
    ...
    r.buttonInput = constrain((int)server.arg(p + "_btn").toInt(), 0, NUM_INPUTS - 1);
...
static void handleReadings() {
  ...
  for (int i = 0; i < NUM_INPUTS; i++) {
    ...
    String label = String(config.inputs[i].name).length()
                       ...
                       : ("Input " + String(i + 1));
    if (config.inputs[i].type == INPUT_ANALOG) {
      if (r.inputOk[i])
        add(label, fmt(r.inputVal[i],
                       config.inputs[i].scale == ANALOG_RAW ? 0 : 2) +
                       aUnit[config.inputs[i].scale]);
    } else {
...
static void handleRaw() {
  ...
  pinMode(PIN_INPUT[ch], INPUT);
  server.send(200, "text/plain", String(analogRead(PIN_INPUT[ch])));
}
